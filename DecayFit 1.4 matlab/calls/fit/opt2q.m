function [res,Lpars,fit] = opt2q(NLpars) 
% Optimization of non-linear pars in quadruple exponential decay
%
%    Input:
...
% Make m x 4 matrix containing each exponential decay component
sim = zeros(length(decay),4);
for i = 2:2:8
...
Aeq = [1 1 1 1]; % Equality constraints: a1 + a2 + a3 + a4 = 1 (Aeq*LP = beq)
beq = 1;       % Equality constraints: a1 + a2 + a3 + a4 = 1
...
Lpars = lsqlin(sim.*repmat(weights,1,4),decay.*weights,[],[],Aeq,beq,lowerLP,upperLP,startLP,options); % Optimize linear parameters
