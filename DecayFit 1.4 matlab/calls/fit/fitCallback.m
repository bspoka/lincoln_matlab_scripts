function fitCallback(handles)
% Callback for pressing fit pushbutton
%
%    Input:
...
try
    ...
    % If global fit, run GlobalFit instead
    ...
    % If optimize shift, run FitShift(handles) instead
    if get(handles.FitShiftCheckbox,'Value') == 1
        fitshiftCallback(handles)
        set(InterfaceObj,'Enable','on')
        return
    end
